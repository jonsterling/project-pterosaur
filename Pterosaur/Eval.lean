...
mutual
  ...
  | .funTp name? A B => .funTp name? (A.eval γ) ⟨γ, B⟩
  | .app M N => Value.apply (M.eval γ) (N.eval γ)
  | .rcdTp locale? selfName? methods => .rcdTp locale? selfName? (methods.close γ)
  ...
  | .rcdTp locale? selfName? spec, .refine _ _ inst =>
    .rcdTp locale? selfName? $ spec.map λ ⟨ℓ, cell⟩ =>
    ⟨ℓ,
  ...
  | _, .abstract C, .appTo arg =>
    ...
    match C.get.destructFunTp with
  ...
  | neu, .abstract C, .call ℓ =>
    ...
    match C.get.destructRcdTp with
    | none =>
      .abstract $ Thunk.pure $
